<script lang="ts">
  ...
  import { writable } from "svelte/store";
    ...
    if (!isWeb3Injected) {
        ...
        errors.update((arr) => [...arr, "⚠️ No extension installed or the user did not accept the authorization ⚠️"]);
    }
    /* else if (!isWeb3Injected && !!window.injectedWeb3) {
        ...
        web3Enable(APP_NAME).then((res) => {
            if (res.length === 0) {
                dAppAccepted.set(false);
                errors.update((arr) => [...arr, "⚠️ Website is denied or pending, you have to allow it. Then refresh the page. ⚠️"]);
                console.log(errors)
            } else {
